...
def test_schedule_and_complete(client, db_session, runner_fake):
    ...
    sched = client.post("/api/transfer/schedule", json={"skill_id": skill_id, "transfer_level": "T2"}).json()
    ...
    # Complete
    comp = client.post(f"/api/transfer/complete/{sched['id']}", json={"result": "success", "confidence": 0.9})
...
def test_invalid_level_422(client, db_session, runner_fake):
    ...
    due = client.get("/api/transfer/due").json()
    assert due
    resp = client.post("/api/transfer/schedule", json={"skill_id": due[0]["skill_id"], "transfer_level": "T9"})
    assert resp.status_code == 422
