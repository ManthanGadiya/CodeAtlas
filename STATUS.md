...
> **Project Status:** 🟢 Levels 1, 2, 3.1, 3.6, 3.4, 3.2, 3.3 & 3.5 Complete — Level 3 Adaptive Intelligence Underway  
> **Current Version:** 0.1.0-dev  
> **Development Stage:** ROADMAP Levels 1-2 complete; Level 3 Phases 3.1 Tutor (0013), 3.6 Retention (0014), 3.4 Curriculum (0015), 3.2 Generator (0016), 3.3 Difficulty (0017), 3.5 Retrieval Practice (0018 retrieval_schedules) landed  
> **Primary Objective:** Level 3 — Transfer Evaluation (3.7) → Level 4 research.

---

# 1. Current State

ROADMAP Levels 1, 2, 3.1, 3.6, 3.4, 3.2, 3.3 and 3.5 are complete. A student can register, browse seeded Python problems, **generate validated variants**, get **a personalized next-problem recommendation**, ask the tutor for socratic hints, see which skills are fading, **practice at the right difficulty**, and **receive scheduled retrieval practice that adapts its ladder and interval from stability** — while every execution, code version, learning event, mistake, behavior, tutor interaction, retrieval attempt, generated problem, difficulty estimate, and retrieval schedule feeds the adaptive loop.

- FastAPI backend: modular monolith (auth, users, problems, execution, events, analytics, skills, mistakes, behavior, **tutor + AI gateway + retention + curriculum + generator + difficulty + retrieval**)
- Next.js frontend: login/bootstrap, dashboard (personalized learner model + **retention due + recommended next + adaptive difficulty + retrieval due**), problem browser (includes generated variants), problem detail with editor + **TutorPanel (hint ladder 0-7)**
- Docker sandboxed execution with CI-verified end-to-end tests
- Immutable learning-event stream + code artifact version chains + analytics appendix + **tutor (0013) + retention (0014) + curriculum decisions (0015) + generator fields (0016) + difficulty vectors (0017) + retrieval schedules (0018) + HINT/RETRIEVAL/CURRICULUM_DECISION/PROBLEM_GENERATED events**
...
- **Retrieval engine**: deliberate scheduling (§20-22: interval = S×0.8, adaptive ×2 / ×0.5 / ×0.8 partial), ladder recognition→transfer ramped by stability, interleaving weakest-first, auto-next scheduling, RETRIEVAL_ATTEMPTED audit
...
Level 3.5 is complete (0018 applied, 10 retrieval tests passing). Next slices per ROADMAP Level 3: **Phase 3.7 Transfer Evaluation** (novel problem recognition + transfer detection, Problem_Generator §49-51) → **Level 4 research** (unified student model, temporal modeling, causal experiments, recommendation policy). Ask before RL/research-grade per AGENTS.md §4.
