...
- Docs hygiene (Option A): renamed `docs/Forgeting_And_Retention.md` → `docs/Forgetting_And_Retention.md` and purged all `Forgeting` references (`backend/app/events/service.py`, `retention/*`, `migrations/0014`, `tests/test_retention.py`); fixed `README.md` `LICENCE` → `LICENSE` link to match actual `LICENSE` file.
- Rate limiter bounded (Option A): `SlidingWindowLimiter` now caps keys at 5000 (LRU), evicts empty/expired buckets via `purge_expired()` + opportunistic `_evict_expired`/`_enforce_key_cap` on `allow()`. Prevents unbounded `per-IP _events` growth behind a reverse proxy (STATUS.md). In-process only remains for single-user; distributed limiter still future work.
- Auth sessions now purgeable: added `purge_expired_sessions(db, grace_seconds)` in `app/auth/service.py` (deletes `expires_at`/`revoked_at` past cutoff). Expired rows were previously checked but never deleted.
- Analytics SQL aggregates (Option A): `build_summary` no longer `SELECT *`s entire execution history; totals via `COUNT() FILTER` aggregates, per-problem via `GROUP BY` (bounded 20), recent activity via `LIMIT 10`. A 10k-history student no longer materializes 10k rows.
- One-command local stack: `docker compose up --build` now starts db + backend + frontend together (ports 5433→5432, 8000, 3000) with healthchecks and `depends_on`. Added `backend/Dockerfile` (python:3.12-slim, auto-migration via `app.main` lifespan, HEALTHCHECK on `/api/healthz`) and `frontend/Dockerfile` (node:20-alpine multi-source build, `NEXT_PUBLIC_API_BASE_URL` arg). Compose backend is wired to `db:5432` internally via `DATABASE_URL` override while host dev keeps `localhost:5433`; frontend builds with `npm run build` and `npm start`. Verified: `docker compose config` clean, `pytest` 188 passing, `ruff check` clean, `npm run build` 5 routes green.
- DB stale-schema bugs (1 & 2): `column problems.fingerprint does not exist` (0016) and `relation "retention_states" does not exist` (0014) no longer 500. `backend/app/main.py` lifespan now runs `alembic upgrade head` idempotently before seeding (fast `connect_timeout=2`, skipped under pytest) and the seed skips under pytest to keep the suite snappy. Added `@app.exception_handler(ProgrammingError)` → 503 with fix hint `cd backend && alembic upgrade head` and `backend/scripts/seed_problems.py` now prints the same hint. Manual fix for an already-running DB: `cd backend && alembic upgrade head`.
...
- **Evaluation harness — Option A hardening (Evaluation_Framework §27-29, §46-48, M23):** new `app/evaluation/` module with pure metrics `brier_score`/`calibration_bins`/`ece`/`learning_gain`/`auc_approx` + baselines `baseline_random`/`baseline_static`/`compare_brier` per AGENTS.md §61 (beat the dumb baseline before claiming intelligence). New `GET /api/evaluation/report` (n, Brier vs naive/static, ECE, calibration bins, comparison) and `GET /api/evaluation/health`; frontend adds `api.evaluationReport()`/`api.evaluationHealth()`. Tests: 17 passing (pure metrics, baselines, auth, empty, after-submits) + `ruff check` clean. This closes the loop needed to *measure* mastery → success calibration instead of asserting it.
- **Unified student state — Level 4.1a (ROADMAP §32, Data_Model §8, M24):** new `app/unified/` deterministic projection — `GET /api/learner/unified-state` (+ `/health`) combining Knowledge (mastery/confidence/gaps/strengths) + Misconceptions (open/recurring) + Behavior (patterns) + Retention (overview/due/score) + Performance (totals/problems/recent) + Preferences + `learning_velocity`/`trend` into one `StudentState`. Pure helpers `compute_overall_mastery`/`compute_confidence`/`compute_learning_velocity`/`compute_retention_score`/`compute_independence_score`. Mirrors aggregates to `student_learning_states` (overall_mastery/learning_velocity/independence_score/retention_score) so the placeholder becomes live. Architecture: API → Service → Domain (existing models), no new tables, event-sourcing friendly. Frontend adds `api.unifiedState()`/`api.unifiedHealth()` and dashboard Unified emerald card (mastery/confidence/retention/independence + velocity/trend + gaps/strong/due). Tests: 13 passing (pure helpers, auth, health, empty, after-submit, persistence mirror) + `ruff check` clean + `npm run build` green. This is the first research-grade Learning State (Data_Model §8).
